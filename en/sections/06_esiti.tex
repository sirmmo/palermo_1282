...
\section{Three-Table Variant}

The canonical configuration is four tables (Conspirators, Barons,
Clergy, People). When only three GMs are available, one faction
sits out: the module handles the reduction well, provided you
rescale the score and narratively offload the absent faction.

\subsection{Score Rescaling}

With three tables the maximum score drops from 16 to 12. The
narrative thresholds rescale proportionally:

\begin{center}
\begin{tabular}{lcc}
\toprule
\cinzel Outcome & \cinzel 4 tables (0--16) & \cinzel 3 tables (0--12) \\
\midrule
The Night of Silence       & 0--4   & 0--3   \\
The Spark Extinguished     & 5--8   & 4--6   \\
The Vespers                & 9--12  & 7--9   \\
The Insurrection           & 13--16 & 10--12 \\
\bottomrule
\end{tabular}
\end{center}

The outcome text itself is unchanged. The Telegram bot shows
\texttt{/total} out of 12 instead of 16.

\secsep

\subsection{Option A --- Without the Barons}

Active tables: \textbf{Conspirators + Clergy + People}.

Baronial pressure does not disappear: it moves off-stage. Alaimo
da Lentini, Donna Margherita Lancia, the Chiaramonte become
interlocutors reached only by letters, rumours, second-hand
reports. The Conspirator PC can ``negotiate with the Lancia''
without a Lancia at the table.

This option keeps three clearly distinct emotional tones: the
conspiracy thriller (Conspirators), the symbolic rite (Clergy),
street-level survival (People). Minimal overlap between tables.

\textbf{Special outcomes to adjust:}
\begin{itemize}[nosep]
  \item \textbf{Barons at 4} is removed. Replacement: if the
    Conspirators reach 4 \textit{and} have at least one documented
    accord with an off-stage baronial house, a significant Sicilian
    house formally changes sides after the uprising.
  \item Other special outcomes remain unchanged.
\end{itemize}

\secsep

\subsection{Option B --- Without the Conspirators}

Active tables: \textbf{Barons + Clergy + People}.

This is the Abulafia reading: no structured conspiracy at the
centre, only three strata of a society at the breaking point.
The Aragonese network exists, but it is invisible to those who
will light the fuse. Giovanni da Procida remains a name that
circulates. Byzantine gold arrives through nameless couriers.

The Conjunction Moment \textit{The Rumour} gains weight: it is
the only window onto the plan moving elsewhere. The coordinating
GM can use it to deliver information no table has planned.

This option pushes the game toward popular spontaneity and noble
strategy; it gives up the spy-thriller register; the emotional
drama grows, because no one knows what is really happening.

\textbf{Special outcomes to adjust:}
\begin{itemize}[nosep]
  \item \textbf{Conspirators at 0} is removed: the Conspirators
    were never on-stage as a faction; their absence is structural,
    not an outcome.
  \item \textbf{Barons at 4} remains and gains narrative weight:
    the Barons are now the only faction with institutional
    authority to give the uprising political shape.
  \item Other special outcomes remain unchanged.
\end{itemize}

\nota[For both options]{
  The Passion layer, the Clergy's \textit{To Bind and to Loose}
  action, and the three Conjunction Moments all work without
  modification: they depend on the tables present, not on the
